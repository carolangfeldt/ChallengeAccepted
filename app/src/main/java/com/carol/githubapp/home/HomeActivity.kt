...
import com.carol.githubapp.R
import com.carol.githubapp.api.Constants
import com.carol.githubapp.api.models.Item
import com.carol.githubapp.repo.RepoActivity
import com.carol.githubapp.utils.Utils
    ...
    override fun onClick(view: View?) {
        when (view?.id) {
            R.id.imageViewSearch -> {
                val ed_t_str :String = editTextSearch.text.toString()
                if(!ed_t_str.isNullOrEmpty())
                    homePresenterImpl.searchRepos(editTextSearch.text.toString(), sortBy, orderBy)
                else
                    this.changeMessage("Please enter some text to make the search!")
                    //Toast.makeText(applicationContext, "Please enter some text to search! ", Toast.LENGTH_SHORT).show()
